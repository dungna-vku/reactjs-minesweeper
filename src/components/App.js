...
export default function App() {
  ...
  // Show all mines
  async function showAllMines() {
    const newField = field;

    for (let index = 0; index < newField.length; index++) {
      if (
        newField[index].isMine ||
        newField[index].status === stateOfCell.FLAGGED
      ) {
        newField[index].status = stateOfCell.OPENED;
      }
    }

    await setField(newField);
  }
